...
function CampaignPage(props) {
  const campaign = props.user.campaigns.find(
    (campaign) => campaign.id === parseInt(props.match.params.id)
  );
  ...
  return (
    <div className="campaign-page">
      ...
      <div className="campaign-players-div">
        <h2>Players</h2>
        {props.loggedIn && currentUserIsOwner(campaign, user) && (
    ...
    return (
      <div key={player.id} className="campaign-player-div">
        ...
        <div className="campaign-player-username">
          <h3>
            <NavLink to={`/${username}`}>{usernameUppercased}</NavLink>
          </h3>
        </div>
      </div>
    );
